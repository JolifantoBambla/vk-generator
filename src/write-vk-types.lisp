...
(defun get-value-setter (member-data struct count-member-names macro-p vk-spec)
  (let ((fixed-slot-name (fix-slot-name (name member-data) (type-name (type-info member-data)) vk-spec)))
    (cond
      ...
      ;; todo: this is broken: <....> is not of type hash-table
      ...
      ((string= "pNext" (name member-data))
       (let ((extending-structs (get-extends struct vk-spec)))
         (if extending-structs
             (format nil "(if (vk:next value)~%                     (translate-next-chain (vk:next value)~%                                           (alexandria:hash-table-plist~%                                            '(~{~(~a~)~}))~%                                           ~(~s~)~%                                           ptr)~%                     (cffi:null-pointer))"
