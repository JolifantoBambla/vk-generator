  ...
  (let ((fixed-slot-name (fix-slot-name (name member-data) (type-name (type-info member-data)) vk-spec))
    ...
    (cond
      ...
      ((gethash (type-name (type-info member-data)) (structures vk-spec))
       (format nil "(vk-alloc:foreign-allocate-and-fill '(:struct %vk:~(~a~)) (~(vk:~a~) ~a) ~a)"
...
(defun write-vk-translators (out struct vk-spec)
  (let ((fixed-type-name (fix-type-name (name struct) (tags vk-spec)))
    ...
    (format out "(defmethod cffi:translate-into-foreign-memory (value (type %vk:c-~(~a~)) ptr)~%  (cffi:with-foreign-slots" fixed-type-name)
    (loop for m in (members struct)
          for i from 1
          do (format out "~%      ~:[  ~;((~]~(%vk:~a~)~:[~;)~]"
                     (= 1 i)
    ...
    (format out "~%       ptr~%       (:struct %vk:~(~a~)))" fixed-type-name)
    (loop for m in (members struct)
          for i from 1
    ...
    (format out "(defmethod cffi:expand-into-foreign-memory (value (type %vk:c-~(~a~)) ptr)~%  `(cffi:with-foreign-slots" fixed-type-name)
    (loop for m in (members struct)
          for i from 1
          do (format out "~%      ~:[  ~;((~]~(~a~)~:[~;)~]"
                     (= 1 i)
    ...
    (format out "(defmethod cffi:translate-from-foreign (ptr (type %vk:c-~(~a~)))~%  (cffi:with-foreign-slots" fixed-type-name)
    (loop for m in (members struct)
          for i from 1
          do (format out "~%      ~:[  ~;((~]~(%vk:~a~)~:[~;)~]"
                     (= 1 i)
