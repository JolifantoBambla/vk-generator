  ...
  (let ((fixed-slot-name (fix-slot-name (name member-data) (type-name (type-info member-data)) vk-spec)))
    (cond
      ...
      ((string= "pNext" (name member-data))
       ...
       (format nil "(let ((slot (vk:next value))) (if slot (vk-alloc:foreign-allocate-and-fill (list :struct (gethash (class-name (class-of slot)) ~a)) slot ptr) (cffi:null-pointer)))"
               *vk-to-%vk-type-name*))
